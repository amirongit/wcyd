...
class MessageRepo(IMessageRepo):
    ...
    async def create(self, source: UniversalPeerIdentifier, target: UniversalPeerIdentifier, content: str) -> None:
        ...
        await self._connection.hmset(
            self._REDIS_KEY_NAMESPACE_.format(identifier=uuid4(), target_identifier=target.peer),
            obj # type: ignore
        )
    ...
    async def relative_to_target(self, identifier: UniversalPeerIdentifier) -> list[Message]:
        ...
        for key in await self._connection.keys(
            self._REDIS_KEY_NAMESPACE_.format(identifier='*', target_identifier=identifier.peer)
        ):
            ...
            Message(
                ...
                source=UniversalPeerIdentifier(
                    node=obj['source_peer'],
                    peer=obj['source_peer']
                ),
                target=identifier,
                content=obj['content']
            )
